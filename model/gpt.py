...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.token_embedding_layer = nn.Embedding(vocab_size, model_dim)
        self.position_embedding_layer = nn.Embedding(context_length, model_dim)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        x = self.token_embedding_layer(context) + self.position_embedding_layer(torch.arange(0, context.shape[1]))

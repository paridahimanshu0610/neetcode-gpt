...
class KVCache:
    ...
    def update(self, new_k: torch.Tensor, new_v: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.cache_k is None:
            ...
            self.cache_k, self.cache_v = torch.cat((self.cache_k, new_k), dim=1), torch.cat((self.cache_v, new_v), dim=1)
...
class CachedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> Tuple[torch.Tensor, KVCache]:
        ...
        full_k, full_v = kv_cache.update(key, value)

        res = ((query @ torch.transpose(full_k, 1, 2)) * (full_k.shape[-1] ** -0.5))
        res = torch.softmax(res, dim = -1) @ full_v

        return torch.round(res, decimals = 4), kv_cache 
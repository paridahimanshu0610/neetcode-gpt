...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.multi_head_layer_list = nn.ModuleList([self.SingleHeadAttention(embedding_dim, head_size) for _ in range(num_heads)])
        self.output_proj_layer = nn.Linear(attention_dim, attention_dim, bias=False) 
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        multi_head_output = torch.cat([single_head_layer(embedded) for single_head_layer in self.multi_head_layer_list], dim = 2)
        return torch.round(self.output_proj_layer(multi_head_output), decimals = 4)

...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.multi_head_layer = self.MultiHeadedSelfAttention(model_dim, num_heads) # Returns batch_size x context_len x model_dim
        self.vanilla_nn_layer = self.VanillaNeuralNetwork(model_dim)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        x = embedded
        x = x + self.multi_head_layer(self.layer_norm(x))
        x = x + self.vanilla_nn_layer(self.layer_norm(x))
